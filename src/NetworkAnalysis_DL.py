...
avg_rows_num = math.floor(rows_num / len(X)) + 30
print(avg_rows_num)
...
def padding(dataset, critical_val, feature_num):
    ...
    for data  in dataset:
        #normalization
        data = preprocessing.normalize(data, norm='l2').tolist()
...
X = padding(X, avg_rows_num, feature_num)
...
class CNN(Module):
    def __init__(self, num_classes, device='cpu'):
        ...
        self.conv_model = Sequential(
            ...
            ReLU(),
            #MaxPool2d(2),
            Conv2d(in_channels=64, out_channels=64,padding=0,  kernel_size=3),
            ReLU(),
...
def loss_batch(model, loss_func, xb, yb, opt=None):
    ...
    num_correct = (pred == yb.long()).sum().item()
    # if test_flag:
    #     # print(out)
    #     print(pred)
    #     print(yb)

    loss = loss_func(out, yb.long())
    if opt is not None:
        loss.backward()
        opt.step()
        opt.zero_grad()
